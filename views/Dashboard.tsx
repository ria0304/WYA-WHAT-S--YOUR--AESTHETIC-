...
import { Box, Sparkles, Sun, Camera, Plane, Leaf, History, User, Palette, Shirt, Layout } from 'lucide-react';
      ...
      <div className="bg-white rounded-t-[60px] p-8 pb-12 shadow-[0_-20px_40px_rgba(0,0,0,0.05)]">
        <div className="grid grid-cols-2 gap-4">
          <MenuButton 
            ...
            label={`Closet (${stats?.wardrobe_count || 0})`} 
          ...
          <div className="col-span-2">
            <button onClick={() => navigate('/travel')} className="w-full bg-indigo-50/50 p-6 rounded-[32px] flex items-center gap-6 hover:bg-indigo-50 transition-colors text-left border border-white shadow-sm">
          ...
          <div className="col-span-2">
            <button onClick={() => navigate('/curate')} className="w-full bg-pink-50/30 p-6 rounded-[32px] flex items-center gap-6 hover:bg-pink-50 transition-colors text-left border border-white shadow-sm">
...
const MenuButton: React.FC<{ icon: React.ReactNode; label: string; bgColor: string; iconColor: string; labelColor?: string; onClick?: () => void; }> = ({ icon, label, bgColor, iconColor, labelColor, onClick }) => (
  <button onClick={onClick} className={`${bgColor} p-6 rounded-[32px] flex flex-col items-center justify-center transition-all active:scale-95 group border border-white shadow-sm hover:shadow-md`}>
